...
for xy in location_dict:
    if location_dict[xy] == 83: #starting point found
...
def find_trails(coords, trail=None):
    ...
    if location_dict[coords] == 69:  # Success condition
        # build_grid(trail)
        ...
        for d in DIRECTIONS:  # Loop through possible directions
            ...
            try:
                if location_dict[coords] == 83:
                    height_diff = ord('a') - location_dict[new_xy]
                else:
                    height_diff = location_dict[new_xy] - location_dict[coords]
                # print(height_diff)
            except KeyError as e:
                # print(e)
                continue
            if height_diff <= 1:  # Check if the new cell is reachable
                if new_xy not in trail:
